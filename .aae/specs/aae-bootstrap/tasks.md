...
- **Evidence:** Mypy and Ruff pass; the 44-test standard-library suite passes, including portable identity, Windows-style normalization, policy-negative, release/record tamper, one-shot outcome, concurrency, semantic, model-routing, lifecycle, history, and telemetry cases; repository validation and compilation pass with 8 skills and 35 capability names; the Python 3.10/3.14 CI matrix is configured; the 0.3.0 wheel builds and installs without network dependencies; the installed CLI initializes 56 files, validates and compiles a fresh project, reproduces canonical accounting output, reports zero configured provider/retriever/model profiles, permits a locally authorized invocation, records its verified outcome, and denies an unauthorized provider without loading the procedure.
