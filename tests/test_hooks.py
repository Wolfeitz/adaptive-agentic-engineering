...
class HookEventTests(unittest.TestCase):
    ...
    def test_adapter_delivery_can_omit_no_match_record_and_add_provenance(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            init_repository(root)
            record, procedures, errors = process_event(
                ...
                idempotency_key="adapter-event-1",
            ...
            self.assertEqual(procedures, {})
            self.assertEqual(record["status"], "no-match")
            self.assertEqual(record["delivery_provenance"]["adapter"], "example")
